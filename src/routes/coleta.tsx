...
import { CheckCircle2, Loader2, Send, Sparkles, Lock, MapPin, Package } from "lucide-react";
      ...
      if (error) {
        if (error.code === "23505") {
          const { data: existingRow } = await supabase
            ...
            .maybeSingle();
          return { recount: !!overrideId, item_code: vals.item_code, row: existingRow ?? null };
        }
        throw new Error(error.message || "Falha ao salvar");
      }

      return { recount: !!overrideId, item_code: vals.item_code, row: inserted };
    },
    onSuccess: (res) => {
      ...
      if (res.row) pushItemToCaches(res.row as Record<string, unknown>);
      qc.invalidateQueries({ queryKey: ["inventory"] });
      if (res.recount) qc.invalidateQueries({ queryKey: ["uc-check"] });

      // Reset Item, UC, Lote — endereço PERMANECE
      setItem("");
      setLote("");
      setUc("");
      setQuantidade("1");
        ...
        <form ref={formRef} onSubmit={handleSubmit} className="space-y-5">
          ...
          {confirm.status === "confirmed" && (
            <div
              role="status"
              aria-live="polite"
              className="flex items-center justify-center gap-2 rounded-lg px-3 py-2 text-sm font-medium bg-success/15 text-success border border-success/30"
            >
